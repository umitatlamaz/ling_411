...
uniform <- as_tibble(runif(120, min = 1, max = 6))

uniform

```

Let us plot the uniform data using a histogram. 

```{r histogram2, fig.cap="Histogram of a uniform distribution.", echo=TRUE}
ggplot(uniform, aes(value)) +
...
ggplot(champs, aes(total_goals)) + geom_histogram() + 
    ...
    geom_text(data=NULL, x = mode_goals-.6, y=1530, label = "mode", color = "green")
