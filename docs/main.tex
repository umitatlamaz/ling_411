...
\includegraphics{main_files/figure-latex/unnamed-chunk-234-2.pdf}
...
\includegraphics{main_files/figure-latex/unnamed-chunk-234-3.pdf}
...
\includegraphics{main_files/figure-latex/unnamed-chunk-237-1.pdf}
...
\includegraphics{main_files/figure-latex/unnamed-chunk-242-1.pdf}
...
\includegraphics{main_files/figure-latex/unnamed-chunk-243-1.pdf}
...
\includegraphics{main_files/figure-latex/unnamed-chunk-246-1.pdf}
...
\includegraphics{main_files/figure-latex/unnamed-chunk-260-1.pdf}

\hypertarget{effect-size-significance}{%
\chapter{Effect Size \& Significance}\label{effect-size-significance}}

Often times, we will observe a difference between the means of two groups. These groups could be variables of any factor. For example, age could be a factor an we could be investigating the difference in voice pitch between children and adults. Or, we could be looking at the difference between the frequency of using the word \textbf{darlamak} between members of Gen X and Gen Z. Yet another example would be looking at the difference between the average valence of smell and taste words.
...
Let us try Cohen's \emph{d} for the Smell and Taste data.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{d }\OtherTok{=}\NormalTok{ (means}\SpecialCharTok{$}\NormalTok{avg[}\DecValTok{1}\NormalTok{] }\SpecialCharTok{{-}}\NormalTok{ means}\SpecialCharTok{$}\NormalTok{avg[}\DecValTok{2}\NormalTok{]) }\SpecialCharTok{/} \FunctionTok{sd}\NormalTok{(senses\_data}\SpecialCharTok{$}\NormalTok{Val)}
\NormalTok{d}
\end{Highlighting}
\end{Shaded}
...
Cohen's \emph{d} is a standard measure of difference in magnitude between two samples. It does not care about the sample size though. Your sample size for each group could be 2, 200,2K, 2M.
...
\begin{longtable}[]{@{}ll@{}}
\toprule()
\textbf{Participant (by shirt color) } & \textbf{Fudnamental Frequency} \\
\midrule()
\endhead
Blue 1 & 175 \\
Blue 2 & 75 \\
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{blue }\OtherTok{=} \FunctionTok{c}\NormalTok{(}\DecValTok{175}\NormalTok{,}\DecValTok{75}\NormalTok{)}
\NormalTok{red }\OtherTok{=} \FunctionTok{c}\NormalTok{(}\DecValTok{312}\NormalTok{,}\DecValTok{190}\NormalTok{)}
...
\NormalTok{\#\# Cohen\textquotesingle{}s d}
\NormalTok{\#\# }
\NormalTok{\#\# d estimate: {-}1.597498 (large)}
\NormalTok{\#\# 95 percent confidence interval:}
\NormalTok{\#\#     lower     upper }
\NormalTok{\#\# {-}6.538998  3.344001}
\end{Highlighting}
\end{Shaded}

We get a large effect. However, there's no way we can conclude that shirt color has anything to do with someone's voice pitch. We cannot generalize to the population. We observe a difference but we probably have a decent error margin in our prediction. To quantify this error, we need to introduce a new mertric \textbf{standard error}.

\hypertarget{standard-error-1}{%
\section{Standard Error}\label{standard-error-1}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{s }\OtherTok{\textless{}{-}}  \FunctionTok{sqrt}\NormalTok{(((}\FunctionTok{sd}\NormalTok{(blue)}\SpecialCharTok{**}\DecValTok{2}\NormalTok{)}\SpecialCharTok{+}\NormalTok{(}\FunctionTok{sd}\NormalTok{(red)}\SpecialCharTok{**}\DecValTok{2}\NormalTok{))}\SpecialCharTok{/}\DecValTok{2}\NormalTok{)}
\NormalTok{s}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{SE }\OtherTok{\textless{}{-}}\NormalTok{ s }\SpecialCharTok{/} \FunctionTok{sqrt}\NormalTok{(}\DecValTok{4}\NormalTok{)}
\NormalTok{SE  }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mean\_diff }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(red) }\SpecialCharTok{{-}} \FunctionTok{mean}\NormalTok{(blue)}

\NormalTok{CI }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(mean\_diff }\SpecialCharTok{{-}}\NormalTok{ (}\FloatTok{1.96}\SpecialCharTok{*}\NormalTok{SE), mean\_diff }\SpecialCharTok{+}\NormalTok{ (}\FloatTok{1.96}\SpecialCharTok{*}\NormalTok{SE))}
\NormalTok{CI}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\#\# [1]  48.70415 203.29585}
\end{Highlighting}
\end{Shaded}
